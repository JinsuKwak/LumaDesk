...
final class AppStateStore: ObservableObject {
    ...
    @Published private(set) var lanPeerStatus = "Off"
    ...
    private let lanPeerService: LANPeerService
    ...
    init(
        ...
        self.lanPeerService = lanPeerService
        ...
        lanPeerService.statusHandler = { [weak self] status in
            self?.lanPeerStatus = status
        }
    }
    ...
    func bootstrap() {
        ...
        applyTheme(preferences.general.theme)
    ...
    func setLANPeerEnabled(_ enabled: Bool) {
        preferences.lanPeer.isEnabled = enabled
        persist()
        lanPeerService.configure(preferences.lanPeer)
    }

    func setLANDeviceName(_ name: String) {
        preferences.lanPeer.deviceName = name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    ...
    func setLANSharedKey(_ key: String) {
        preferences.lanPeer.sharedKey = key.trimmingCharacters(in: .whitespacesAndNewlines)
        persist()
        lanPeerService.configure(preferences.lanPeer)
    }
    ...
    private func registerGlobalHotKeys() {
        ...
        hotKeyRegistrationError = failures.isEmpty
            ? nil
            : "Shortcuts already in use: \(failures.joined(separator: ", "))"
    }
}
