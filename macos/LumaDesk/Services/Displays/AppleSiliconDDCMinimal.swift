...
struct AppleSiliconDDCDisplay {
    ...
    var sharedID: String {
        let serial = !alphanumericSerialNumber.isEmpty
            ? alphanumericSerialNumber
            : (serialNumber == 0 ? "" : String(serialNumber))
        guard !manufacturerID.isEmpty, !productName.isEmpty, !serial.isEmpty else {
            return "LOCAL:\(id.uppercased())"
        }

        return "EDID:\(canonical(manufacturerID)):\(canonical(productName)):\(canonical(serial))"
    }
